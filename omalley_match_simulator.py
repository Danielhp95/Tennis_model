...
class MatchSimulator(class):
    ...
    """
    def __innit__(self, game_f=omalley.G, tie_break_f=omalley.TB):
        self.game_f      = game_f
        self.tie_break_f = game_f
        ...
        while not is_match_over(score):
            ...
            while not is_set_over(game_score):
                ...
                if is_TB_game(game_score):
                    p = tie_break_f(mean_a, mean_b)
                else:
                    p = game_f(serve_p_a) if player_a_serves else omalley.G(1-serve_p_b)
